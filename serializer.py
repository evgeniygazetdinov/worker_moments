...
class TVProgrammsSerializer(serializers.ModelSerializer):
    ...
    date = serializers.SerializerMethodField(method_name='start_date')
...
class TVProgramsSchedulerSerializer(TVProgrammsSerializer):
    ...
    def button_schedule(self,obj):
        if obj.program.can_schedule == True:
            ...
            #add to each word % for sticky in url
            button_titles = "Посмотреть%<<{title}>>на%Канале%Disney%по%телевизору!".format(title = "%20".join(str(obj.program.title.decode()).split()))
...
class ScheduleApiSerializer(serializers.ModelSerializer):
    ...
    def programs_on_day(self,obj):
        ...
        serializer = TVProgramsSchedulerSerializer(instance=qs, many=True,context =self.context)
